...
void run_tests(int queue_impl, int queue_size, int hold, char *dist_filepath,
               int num_ops, int num_trials, char *save_name);
...
int main(int argc, char *argv[]) {
  ...
  printf("queue_implementation = %s, queue_size = %d, hold = %d, dist_filepath "
         "= %s, save_filepath = %s, num_ops = %d, num_trials = %d\n",
         impl, queue_size, hold, dist_filepath, save_filepath, num_ops,
         num_trials);
  printf("--------->\n");
  ...
  run_tests(queue_impl, queue_size, hold, dist_filepath, num_ops, num_trials,
            save_filepath);
  return 0;
}
...
void run_tests(int queue_impl, int queue_size, int hold, char *dist_filepath,
  ...
  if (hold) {
    ...
    for (l = 0; l < num_trials; l++) {
      ...
      queue_impl == 0 ? reset_list() : reset_heap();
      for (j = 0; j < queue_size; j++) {
        queue_impl == 0 ? enqueue_list(sample_list[j])
                        : enqueue_heap(sample_list[j]);
      }
      ...
      for (i = 0; i < num_ops; i++) {
        // DQ and throw away
        queue_impl == 0 ? dequeue_list() : dequeue_heap();
        // Enqueue new item
        queue_impl == 0 ? enqueue_list(sample_list[j])
                        : enqueue_heap(sample_list[j]);
        j++;
